define(['require', './normalize'], function (req, normalize) {
  ...
  function compress(css) {
    ...
    if (typeof process !== "undefined" && process.versions
      ...
      try {
        ...
        console.log(
            'Compression module not installed. Use "npm install csso -g" to enable.');
        return css;
      }
      ...
      console.log(
          'Compressed CSS output to ' + Math.round(css.length / csslen * 100)
          + '%.');
      return css;
    }
  ...
  function saveFile(path, data) {
    if (typeof process !== "undefined" && process.versions
      ...
      var output = new java.io.BufferedWriter(
  ...
  cssAPI.load = function (name, req, load, _config) {
    ...
    if (!siteRoot) {
      siteRoot = path.resolve(config.dir || path.dirname(config.out),
              config.siteRoot || '.') + '/';
      if (isWindows) {
  ...
  cssAPI.onLayerEnd = function (write, data) {
    ...
    if (config.separateCSS) {
      ...
      process.nextTick(function () {
        if (global._requirejsCssData) {
          css = global._requirejsCssData.css = css
              + global._requirejsCssData.css;
